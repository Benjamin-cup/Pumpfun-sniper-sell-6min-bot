...
import { retrieveEnvVariable } from "../../sdksrc/utils"

const TIME_DELAY = Number(retrieveEnvVariable('TIME_DELAY'))
...
async function buyToken(
  ...
  try {
    ...
    timestamp1 = Date.now();
    console.log("time delay==========>", timestamp1);

  } catch (error) {
    console.error(error);
    return false
  }
  // const MINT_ADDRESS = "CERk2PDGpqtAdvsswUqseiQA8x9Gb2bEv2B7ULRypump"
  ...
  try {
    ...
    setTimeout(async () => {
      timestamp2 = Date.now();
      console.log("time delay==========>", timestamp2);

      await sellToken(mint, keypair, connection); // Assuming sellToken is an async function
      console.log("snipe");
    }, (TIME_DELAY * 1000));
